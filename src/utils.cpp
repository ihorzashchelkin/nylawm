              ...
              const xcb_generic_error_t* error)
{
  std::println(
    ...
    xcb_errors_get_name_for_error(error_context, error->error_code, nullptr));
}

void
PropertyRemoveAtom(xcb_connection_t* Conn,
                   xcb_window_t Win,
                   xcb_atom_t Property,
                   xcb_atom_t Val)
{
  ...
  xcb_change_property(
    Conn, XCB_PROP_MODE_REPLACE, Win, Property, XCB_ATOM_ATOM, 32, c, NewVals);
  xcb_flush(Conn);
  free(Reply);
}

void
PropertyAddAtom(xcb_connection_t* Conn,
                xcb_window_t Win,
                xcb_atom_t Property,
                xcb_atom_t Val)
{
...
bool
PropertyHasAtom(xcb_connection_t* Conn,
                xcb_window_t Win,
                xcb_atom_t Property,
                xcb_atom_t Val)
{
...
void
PropertyToggleAtom(xcb_connection_t* Conn,
                   xcb_window_t Win,
                   xcb_atom_t Property,
                   xcb_atom_t Val)
{
